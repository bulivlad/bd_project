...
<head>
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.1/js/bootstrap.min.js"></script>
    ...
    <%
      List<CustomerPreference> customerPreferenceList = (List<CustomerPreference>) request.getAttribute("customerPreferenceList");
      List<CarForSale> carForSaleList = (List<CarForSale>) request.getAttribute("carForSaleList");
    %>
      ...
      <table id="dataTable" style="width:60%" border="0" align="center">
        ...
        <tr>
          ...
          <td>
            ...
            <select name="carFeatureId" required="required">
              <%
                for (CustomerPreference customerPreference : customerPreferenceList){
              %>
                <option value="<%=customerPreference.getId()%>"><%=customerPreference.getCustomerPrefDetails()%></option>
              <%
                }
              %>
            </select>
        ...
        <tr>
          <td>
            <div class="label label-info">Id masina*: </div>
              <select name="carFSId" required="required">
                  <%
                      for(CarForSale carForSale : carForSaleList){
                  %>
                        <option value="<%=carForSale.getId()%>"><%=carForSale.getManufacturerName()%> <%=carForSale.getModelName()%> - <%=carForSale.getVehicleCategory()%></option>
                  <%
                      }
                  %>
              </select>
